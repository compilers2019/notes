    ...
    BEGIN
      CASE CurrentCh(bp0) OF
        'B': IF Equal('BEGIN') THEN BEGIN sym := 5; 
             END;
        'C': IF Equal('CONDITION') THEN BEGIN sym := 9; 
             END;
        'E': IF Equal('ELSE') THEN BEGIN sym := 13; 
             END ELSE IF Equal('END') THEN BEGIN sym := 15; 
             END ELSE IF Equal('ENDIF') THEN BEGIN sym := 14; 
             END;
        'F': IF Equal('FOREVER') THEN BEGIN sym := 10; 
             END;
        'I': IF Equal('IF') THEN BEGIN sym := 11; 
             END;
        'P': IF Equal('PROGRAM') THEN BEGIN sym := 4; 
             END;
        'R': IF Equal('REPEAT') THEN BEGIN sym := 6; 
             END;
        'T': IF Equal('THEN') THEN BEGIN sym := 12; 
             END;
        'U': IF Equal('UNTIL') THEN BEGIN sym := 8; 
             END;
        'o': IF Equal('otherstatement') THEN BEGIN sym := 7; 
             END;
      ELSE BEGIN END
      END
    END;
